...
export class CustomersController {
    ...
    @Post(':id/payments')
    recordPayment(@Param('id') id: string, @Body() body: { amount: number; paymentMode: string; note?: string }) {
        return this.customersService.recordPayment(id, body);
    }

    @Post(':id/ledger')
    recordLedgerEntry(@Param('id') id: string, @Body() body: { amount: number; type: 'DEBIT' | 'CREDIT'; description: string; paymentMode?: string }) {
