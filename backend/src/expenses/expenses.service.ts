...
export class ExpensesService {
  ...
  async create(data: any, userId: string) {
    ...
    await this.auditLog.createLog(
      userId,
      'EXPENSE_CREATE',
      `Submitted expense for approval: ${expense.title} (${expense.amount})`,
    );
    return expense;
  }
  ...
  async approve(id: string, adminId: string, note?: string) {
    return this.prisma.$transaction(async (tx) => {
      ...
      await this.auditLog.createLog(
        adminId,
        'EXPENSE_APPROVE',
        `Approved expense: ${expense.title} (${expense.amount}). Note: ${note || '-'}`,
      );
  ...
  async reject(id: string, adminId: string, note?: string) {
    return this.prisma.$transaction(async (tx) => {
      ...
      await this.auditLog.createLog(
        adminId,
        'EXPENSE_REJECT',
        `Rejected expense: ${expense.title} (${expense.amount}). Note: ${note || '-'}`,
      );
