...
export class CouponsService {
  ...
  async validateCoupon(
    ...
    if (coupon.customerId !== customerId)
      throw new BadRequestException("Coupon doesn't belong to this customer");
    ...
    if (billingType !== 'WHOLESALE')
      throw new BadRequestException(
        'Coupons can only be applied to Wholesale bills',
      );
    if (paymentMode === 'CREDIT')
      throw new BadRequestException(
        'Coupons cannot be used with Credit payment',
      );
    if (eligibleAmount < coupon.minEligibleAmount) {
      throw new BadRequestException(
        `Minimum eligible amount (excluding tobacco) must be ₹${coupon.minEligibleAmount}`,
      );
    }
