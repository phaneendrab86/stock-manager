...
export class VisitsService {
  ...
  async create(data: any, userId: string) {
    ...
    await this.auditLog.createLog(
      userId,
      'VISIT_CREATE',
      `Recorded visit: ${visit.distributor.name} by ${visit.salesman.name}`,
    );
    return visit;
  }
  ...
  async update(id: string, data: any, userId: string) {
    ...
    await this.auditLog.createLog(
      userId,
      'VISIT_UPDATE',
      `Updated visit record: ${visit.distributor.name}`,
    );
    return visit;
  }

  async remove(id: string, userId: string) {
    const visit = await this.prisma.salesmanVisit.delete({
      where: { id },
    ...
    await this.auditLog.createLog(
      userId,
      'VISIT_DELETE',
      `Deleted visit record for: ${visit.distributor.name}`,
    );
    return visit;
  }
