...
export class InvoicesService {
    ...
    async create(data: any) {
        const { items, customerId, paymentMode, discount, totalAmount, billingType, couponCode } = data;
        ...
        if (customerId && paymentMode !== 'CREDIT') {
            const eligibilityCheck = await this.rewardSettingsService.checkEligibility(
                customerId,
                paymentMode,
                billingType,
                eligibleAmount,
        ...
        if (couponCode && customerId) {
            ...
            appliedCouponAmount = Math.min(eligibleAmount * (couponToUse.discountValue / 100), couponToUse.discountCap);
        }

        const netAmount = totalAmount - (discount || 0) - appliedCouponAmount - rewardDiscount;
        ...
        return this.prisma.$transaction(async (tx) => {
            ...
            if (paymentMode === 'CREDIT' && customerId) {
                ...
                await tx.customerLedger.create({
                    data: {
                        ...
                        debit: netAmount,
                        balance: (await tx.customer.findUnique({ where: { id: customerId } }))?.outstandingBalance || 0,
                        paymentMode: 'CREDIT',
            ...
            if (billingType === 'WHOLESALE' && paymentMode !== 'CREDIT' && eligibleAmount >= 2000 && customerId) {
            ...
            if (giftAllocations.length > 0 && customerId) {
                const updatedInvoice = await this.giftAllocationService.executeAllocation(
                    invoice.id,
                    customerId,
                    giftAllocations,
