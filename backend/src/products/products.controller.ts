...
export class ProductsController {
    ...
    adjustStock(
        @Param('id') id: string,
        @Body() body: { quantity: number; unitId: string; type: string; note?: string },
        @Request() req: any
    ) {
        return this.productsService.adjustStock(id, body.quantity, body.unitId, body.type, req.user.userId, body.note);
    }
